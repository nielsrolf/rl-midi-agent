...
train_generator = RandomMidiDataGenerator(real_midis[:50], mel_lstm.preprocess, mapper, 4)
test_generator = RandomMidiDataGenerator(real_midis[50:60], mel_lstm.preprocess, mapper, 10, max_num_timeframes=1000)

history = model.fit_generator(train_generator(), validation_data=test_generator(),
                              steps_per_epoch=train_generator.steps_per_epoch, epochs=2, validation_steps=test_generator.steps_per_epoch, validation_freq=1, verbose=1)
